...
| D-14 | App updates | `tauri-plugin-updater` + GitHub Releases | In-app "Update available" notification (startup check, silent unless found; manual check via Help > Check for Updates…, #180). Update artifacts are signed with a minisign keypair (`TAURI_SIGNING_PRIVATE_KEY`/`_PASSWORD` GitHub secrets; public key in `tauri.conf.json`'s `plugins.updater.pubkey`) so the app verifies authenticity before installing. The manifest (`latest.json`) is generated by `release.yml` from the NSIS `.sig` output and published as a release asset; the endpoint is the stable `.../releases/latest/download/latest.json` URL. Installing runs the NSIS installer in passive mode (`plugins.updater.windows.installMode`), which never shows the interactive "uninstall previous version?" prompt a manually double-clicked installer does |
...
**Scan-root management** (#186): `GET /scan/roots` (added with multi-root support, #59) lists every configured root — `path`, `is_network`, `last_seen_at`, and a live `reachable` flag from `check_reachable()`. `DELETE /scan/roots/{id}` stops scanning a root going forward by deleting only its `scan_roots` row — no `photos`/`faces`/`people` row is ever touched, so already-indexed data from that folder survives untouched (consistent with the "never silently delete user data" rule; nothing else in the schema references `scan_roots.id` as a foreign key, so no cascading is needed or possible). The frontend's `ScanRootsView` (··· menu → "Manage Scan Folders…") lists roots with a network/local tag and last-scanned time, reuses the existing add-folder flow for "Add Folder…", and requires an inline confirm before removing a row. A follow-up "also remove the photos that came from this folder" action is a possible future issue — intentionally out of scope here.
